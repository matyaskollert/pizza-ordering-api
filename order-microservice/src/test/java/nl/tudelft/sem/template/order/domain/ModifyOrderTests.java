...
import nl.tudelft.sem.template.order.domain.product.*;
...
public class ModifyOrderTests {
    ...
    @Test
    public void testAddProductCorrectName() throws Exception {
        //Add a product
        //Expect: the product has the correct name in the response
        Item productItem = new Item("Margherita", List.of(1, 2), false, 1.0);
        Item toppingItem = new Item("Pepperoni", List.of(3), false, 2.0);
        Order mockOrder = new Order();

        OrderHelperService mockHelperService = mock(OrderHelperService.class);
        AddToOrderService mockAdd = new AddToOrderService(orderService, productService,
                mockProductMicroserviceAdapter, mockHelperService);
        ...
        when(orderService.getOrder(2)).thenReturn(mockOrder);
        BasicProductInfo productInfo = new BasicProductInfo();
        productInfo.setName("Margherita");
        productInfo.setToppings(List.of("Pepperoni"));

        List<Product> result = mockAdd.handleAddingProduct(mockOrder, List.of(productInfo), "");
    ...
    @Test
    public void testRemoveNullTopping() throws Exception {
        //Product contains only one topping, which we remove
        //Expect: removing topping is null, it throws InvalidToppingsException
        Item productItem = new Item("Margherita", List.of(1, 2), false, 1.0);
        Order mockOrder = new Order();
        Product product = new Product(
                "Margherita",
        ...
        Exception e = assertThrows(InvalidToppingsException.class, ()
                -> modifyOrderService.removeToppingsFromProduct(1L, null, ""));
